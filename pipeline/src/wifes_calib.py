...
def extract_wifes_stdstar(cube_fn,
    ...
    print("init_obj_cube_var:",init_obj_cube_var)
    print("lam_array", lam_array)
...
def derive_wifes_calibration(cube_fn_list,
    ...
    for i in range(len(cube_fn_list)):
        ...
        try:
          # what is this 'rr'? this is forcing an except error causing it to
          # forcefully add ref_fname with '.tab' file extension
          # instead of looking for it in ref_fname_lookup.keys()

          #if rr != None:
          #    ref_fname = ref_fname_list[i]
          if star_name in ref_fname_lookup.keys():
              print('>> FOUND! star_name:', star_name)
            #############=================================================#############
            #############=================================================#############
              ref_fname =   star_name
              # ref_fname =   [star_name]
            ...
            #############=================================================#############
            # ref_fname = "TEST"
              print('>> oh no! no star_name')
              continue
            #############=================================================#############
            #############=================================================#############
        except:
          print("Unable to find standard star in defaults")
          ref_fname =   star_name

        # get observed data

        print('>> ref_fname:', ref_fname)
        ref_fname = ref_fname.lower()
        ref_fname += ".tab"
        print('>> final ref_fname:', ref_fname)
        ...
        print("init_flux_ratio:", init_flux_ratio)
        print("=============")
