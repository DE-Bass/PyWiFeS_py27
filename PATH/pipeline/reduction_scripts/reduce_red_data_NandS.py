...
#------------------------------------------------------
# Sky subtraction!
def run_sky_sub_ns(ns_proc_list, prev_suffix, curr_suffix):
...
def run_sky_sub_std(std_proc_list, prev_suffix, curr_suffix):
    for fn in std_proc_list:
        in_fn  = '%s%s.p%s.fits' % (out_dir, fn, prev_suffix)
        out_fn = '%s%s.p%s.fits' % (out_dir, fn, curr_suffix)
...
def run_sky_sub(metadata, prev_suffix, curr_suffix):
    ...
    # This needs to be refactored to take into account multiple cases

    # Case 1 standard and science were observed with N&S
    # Case 2 standard was observed and classical unequal and science were observed with N&S
    # Case 3 standard and science were observed with classical unequal
    # Case 2 is the default for DEbass but 1 and 3 may happen
    ...
    if len(std_proc_list) > 0:
        run_sky_sub_std(std_proc_list, prev_suffix, curr_suffix)

    if len(offset_proc_list) > 0:
        ...
        if local_arcs :
            ...
            if len(local_arcs) ==2:
                ...
                if ds1>0 and ds2>0:
                    ...
                    fits0.writeto(wsol_fn, overwite=True)
...
def run_derive_calib(metadata, prev_suffix, curr_suffix, method = 'poly',**args):
    ...
    extract_list = ['%s%s.x%s.dat' % (out_dir, fn, prev_suffix)
                    for fn in std_obs_list]
    print 'Deriving sensitivity function, using %s' % extract_list
